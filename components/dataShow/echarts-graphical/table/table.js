// components/dataShow/echarts-graphical/table/table.js
Component({
    /**
     * 组件的属性列表
     */
    ...
    attached() {
        let {typecode,answerOption} = this.data
        let newArr= []  
        if(typecode === 6)
        {
            answerOption.forEach((v,i) => {
                let newObj = {
                  chooseNum: v.chooseNum,
                  optionContent: v.columnContent
                }
                newArr.push(newObj)
               })
               answerOption = newArr
